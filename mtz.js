...
const MTZ = {
    ...
    myPage: function () {
        ...
        // // // 那个页面有问题。必须点两次。才能确保切换到那个位置
        // let myPageEle = AutojsUtil.getEleBySelectorWithAutoRefresh(text("我的主页"), "我的主页", 10, "微信", function () {
        //     WeiXin.refreshWeb()
        // })

        // if (myPageEle) {
        //     log("点击 我的主页")
        //     AutojsUtil.clickEle(myPageEle.parent())
        // }

        // sleep(1000)
        // // // 那个页面有问题。必须点两次。才能确保切换到那个位置
        // myPageEle = AutojsUtil.getEleBySelectorWithAutoRefresh(text("我的主页"), "我的主页", 10, "微信", function () {
        //     WeiXin.refreshWeb()
        // })
    ...
    helpEach: function () {
        ...
        // sleep(1.5 * 1000)
        AutojsUtil.clickSelectorWithAutoRefresh(text("互助活动"), "互助活动", 10, "微信", function () {
            WeiXin.refreshWeb()
        })
    },
    ...
    activeRead: function () {
        ...
        let ele = AutojsUtil.getEleBySelectorWithAutoRefresh(text("文章阅读推荐"), "文章阅读推荐", 10, "微信", function () {
            WeiXin.refreshWeb()
        })
        ...
        while (1) {
            ...
            if (eles && eles.length > 0) {
                ...
                if (leftTimeEle) {
                    let miniteStr = leftTimeEle.getText().replace("剩余", "").replace("分钟", "")
                    let futureTime = parseInt(miniteStr) * 1000 * 60 + new Date().getTime()
    ...
    getQrPosition: function () {
        ...
        let qrY = t1.bounds().top + parseInt((t2.bounds().top - t1.bounds().top) / 2)
    ...
    payout2WeiXin: function () {
        log("提现到微信")
        AutojsUtil.clickSelectorWithAutoRefresh(text("提现到微信"), "提现到微信", 10, "微信")
    },
    transferScore: function () {
        log("积分转移")
        AutojsUtil.clickSelectorWithAutoRefresh(text("积分转移"), "积分转移", 10, "微信")
        sleep(1000)
        log("再次点击 积分转移")
